...
foreach ($productosPaginados as $producto) { 
    if($m == "Añadir"){
        echo "<tr>
        <td>
            <center><button id='añadir{$producto['id']}' class='btn btn-success btn-sm' onclick='crearTicket({$producto['id']}, {$producto['nombre']}, {$producto['precio']}, {$producto['descuento']})'>+</button></center>
        </td>
